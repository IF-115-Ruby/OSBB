...
class ShowOsbb extends React.Component {
    ...
    componentDidMount() {
      fetch(`/api/v1/admin/osbbs/` +this.props.id+ `.json`)
        .then((response) => {
    ...
    mapImg = () => {
      let src = `https://maps.googleapis.com/maps/api/staticmap?zoom=17&size=500x300&markers=size:small%7Ccolor:red%7C${this.state.osbb.address.coordinates.latitude},${this.state.osbb.address.coordinates.longitude}&key=AIzaSyB79BaHbHY_1floWCNQ1nlSx78zj6CYG04`
      return ( <
        img src = {
          src
        }
...
function ReturnAddress(address) {
  if (address.full_address) {
    ...
    <div className="osbb-content">
      <div className="content-center">
        <span className="item-value">Address is not set</span>
      </div>
      <div className="map-osbb">
        <div className="content-center">
          <span className="item-value"><i className="fas fa-map-marked-alt fa-3x"></i></span>
        </div>
      </div>
    </div>
  )
