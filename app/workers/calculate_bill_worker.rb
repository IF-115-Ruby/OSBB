class CalculateBillWorker
  ...
  def perform(*_args)
    BillingContract.all.each do |billing_contract|
      meter_reading = meter_reading(billing_contract)
      bill_value = bill_value(meter_reading, billing_contract)
  ...
  def meter_reading(billing_contract)
  ...
  def bill_value(meter_reading, billing_contract)
    delta_meter_reading = delta_reading(meter_reading, billing_contract) if meter_reading
    bill_value = calculate_bill(delta_meter_reading, billing_contract)
    value_depending_payments(bill_value, billing_contract)
  end

  def delta_reading(meter_reading, billing_contract)
  ...
  def value_depending_payments(value, billing_contract)
